...
n_cpu = 0
train_dataloader = DataLoader(train_dataset, batch_size=10, shuffle=True, num_workers=n_cpu)
valid_dataloader = DataLoader(valid_dataset, batch_size=10, shuffle=False, num_workers=n_cpu)
test_dataloader = DataLoader(test_dataset, batch_size=10, shuffle=False, num_workers=n_cpu)
...
model = SegTreeModel("FPN", "resnet34", in_channels=3, out_classes=1)

checkpoint_callback = ModelCheckpoint(save_top_k=2, monitor="valid_dataset_iou", mode="max", dirpath="checkpoints/", filename="chekpoint-FPN-resnet34-{epoch:02d}-{valid_dataset_iou:.4f}")
